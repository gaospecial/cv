...
# When in export mode the little dots are unaligned, so fix that. 
if(PDF_EXPORT){
  cat("View full CV online with links at _r.bio-spring.info/cv_")
}
```
...
View online of the full version CV at [r.bio-spring.info/cv](https://r.bio-spring.info/cv/).
...
### Regulation of synthetic bacteria community
...
```{r}
print_section(position_data, 'corresponding-author')
```

## First (and co-first) Author

```{r}
print_section(position_data, 'first-author')
```

## Book Chapter

```{r}
print_section(position_data, 'book-chapter')
```
